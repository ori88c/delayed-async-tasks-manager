...
exports.DelayedAsyncTasksManager = void 0;
/**
 ...
 */
class DelayedAsyncTasksManager {
    constructor() {
        this._keyToExecutionInfo = new Map();
        this._uncaughtErrors = [];
        this._pendingTasksCount = 0;
        this._currentlyExecutingTasksCount = 0;
    }
    /**
     * pendingTasksCount
     *
     ...
     */
    get currentlyExecutingTasksCount() {
     ...
     */
    addTask(uniqueKey, task, delayMs) {
        let executionInfo = this._keyToExecutionInfo.get(uniqueKey);
        if (executionInfo) {
            const isPending = executionInfo.timer !== undefined;
            throw new Error(`DelayedAsyncTasksManager cannot schedule key ${uniqueKey} as there is a ` +
        ...
        const executeAndUpdateInternalState = async () => {
            try {
                await task();
            }
            catch (err) {
     ...
     */
    async awaitTaskCompletion(uniqueKey) {
        const executionInfo = this._keyToExecutionInfo.get(uniqueKey);
        if (!executionInfo) {
            // Either never scheduled, or already completed.
            return false;
     ...
     */
    abortAllPendingTasks() {
        const abortedKeys = []; // To avoid deletion during iteration, as it may corrupt the iterator.
     ...
     */
    async awaitCompletionOfAllCurrentlyExecutingTasks() {
...
exports.DelayedAsyncTasksManager = DelayedAsyncTasksManager;
//# sourceMappingURL=delayed-async-tasks-manager.js.map