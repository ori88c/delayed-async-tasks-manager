...
const delayed_async_tasks_manager_1 = require("./delayed-async-tasks-manager");
/**
 * resolveFast
 *
...
describe('DelayedAsyncTask tests', () => {
    ...
    describe('Happy path tests', () => {
        test('validates task status getters throughout the lifecycle of a single task, from scheduling to completion', async () => {
            ...
            const manager = new delayed_async_tasks_manager_1.DelayedAsyncTasksManager();
            ...
            let taskResolveCallback;
            const task = () => new Promise(res => taskResolveCallback = res);
        ...
        test('mixed scenario: getters should accurately reflect the number of pending and executing tasks', async () => {
            ...
            const getTaskKey = (ithTask) => `UNIQUE_TASK_KEY_${ithTask}`;
            const manager = new delayed_async_tasks_manager_1.DelayedAsyncTasksManager();
            for (let ithTask = 1; ithTask <= totalTasksCount; ++ithTask) {
                ...
                const task = () => new Promise(res => taskResolveCallbacks.push(res));
                manager.addTask(key, task, delayMs);
            ...
            for (let ithTask = executingTasksCount; ithTask > 0; --ithTask) {
                const key = getTaskKey(ithTask);
                expect(manager.isExecuting(key)).toBe(true);
                const awaitTaskCompletionPromise = manager.awaitTaskCompletion(key);
        ...
        test('awaitCompletionOfAllCurrentlyExecutingTasks should resolve only after all currently executing ' +
            ...
            for (let ithTask = 1; ithTask <= totalTasksCount; ++ithTask) {
                ...
                if (ithTask <= successfulTasksCount) {
                    task = () => new Promise(res => taskResolveCallbacks.push(res));
                }
                else {
                    task = () => new Promise((_, rej) => taskRejectCallbacks.push(rej));
                }
                manager.addTask(key, task, delayMs);
                expect(setTimeoutSpy).toHaveBeenCalledTimes(ithTask);
            }
            ...
            const awaitCompletionOfAllTasks = (async () => {
                await manager.awaitCompletionOfAllCurrentlyExecutingTasks();
        ...
        test('abortAllPendingTasks should cancel all pending tasks', async () => {
            ...
            const getIthKey = (ithTask) => `MOCK_KEY_${ithTask}`;
            const manager = new delayed_async_tasks_manager_1.DelayedAsyncTasksManager();
            for (let ithTask = 1; ithTask <= totalTasksCount; ++ithTask) {
                manager.addTask(getIthKey(ithTask), () => Promise.resolve(), delayMs);
                expect(setTimeoutSpy).toHaveBeenCalledTimes(ithTask);
                expect(manager.currentlyExecutingTasksCount).toBe(0);
            }
        ...
        test('awaitTaskCompletion should resolve only when the task completes: ' +
            ...
            let successfulTaskResolveCallback;
            const rejectingTaskKey = "REJECTING_TASK_KEY";
            const rejectingTaskDelay = successfulTaskDelay + 2345;
            let rejectingTaskRejectCallback;
            const manager = new delayed_async_tasks_manager_1.DelayedAsyncTasksManager();
            manager.addTask(successfulTaskKey, () => new Promise(res => successfulTaskResolveCallback = res), successfulTaskDelay);
            expect(setTimeoutSpy).toHaveBeenCalledTimes(1);
            manager.addTask(rejectingTaskKey, () => new Promise((_, rej) => rejectingTaskRejectCallback = rej), rejectingTaskDelay);
            ...
            const awaitSuccessfulTaskCompletionPromise = (async () => {
            ...
            const awaitRejectingTaskCompletionPromise = (async () => {
    ...
    describe('Negative path tests', () => {
        test('should throw an error when the scheduled task corresponds to a currently managed key', async () => {
            ...
            expect(setTimeoutSpy).toHaveBeenCalledTimes(1);
            expect(() => manager.addTask(existingKey, () => Promise.resolve(), delayMs)).toThrow();
        ...
        test('should capture uncaught exceptions thrown during task execution', async () => {
            ...
            const manager = new delayed_async_tasks_manager_1.DelayedAsyncTasksManager();
            const expectedUncaughtErrorsInOrder = [];
            const createTaskError = (ithTask) => ({
                message: `Mock error message no. ${ithTask}`,
                taskID: ithTask
            });
            for (let ithTask = 1; ithTask <= totalTasksCount; ++ithTask) {
                const key = ithTask;
                const task = () => Promise.reject(createTaskError(ithTask));
...
//# sourceMappingURL=delayed-async-tasks-manager.test.js.map